    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-md border-b border-border/50">
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6 font-medium text-muted-foreground">
          ...
          <Link href="#results" className="hover:text-primary transition-colors">{t('nav.results')}</Link>
          <LanguageSwitcher />
          <Link href="/register" className="bg-primary text-white px-6 py-2.5 rounded-full hover:bg-primary/90 transition-all soft-glow">{t('nav.start')}</Link>
        </div>
